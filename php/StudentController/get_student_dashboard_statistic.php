...
//--------------- points  ----------------------
$sql_points = "SELECT points FROM users WHERE user_id = $user_id"; 
...
{ 
    ...
    $student_points = $row['points']; // target to the row "points" in the table
}
else 
{
    $student_points = 0;
...
// ---------------- total recycling -----------------
$sql_weight = "SELECT SUM(weight) as total_kg FROM logs WHERE user_id = $user_id AND status = 'approved'"; //sum up the weight for the display if the logs is approved
...
$sql_event= "SELECT COUNT(*) as count FROM registrations WHERE user_id = $user_id"; 
